...
### 2026-04-12 (Windows 작업분)
- 설정 메인 화면을 기능 허브형에서 `상태 요약 + 핵심 설정` 중심으로 재구성
- 메인 설정에서는 투자/통계/전체 메뉴 같은 우회 진입을 빼고, 알림/텔레그램/Google 상태를 실제 연결 기준 문구로 표시
- 앱 관리의 데이터 삭제는 별도 `위험 작업` 접힘 구역으로 이동
- 검증: `npm run build` 통과

### 2026-04-12 (Windows 작업분)
- 홈 `오늘 할일` 카드에 편집 진입 없이 바로 추가하는 입력 영역을 넣고 시간/중요도를 즉시 지정할 수 있게 정리
- 홈 `오늘 습관` 카드는 진행/완료 요약 중심으로 줄이고, 완료 항목은 기본적으로 접어 남은 습관 위주로 보이게 조정
- 커뮤니티 보호 방 입장 암호 확인은 `window.prompt` 대신 인앱 모달로 교체
- 검증: `npm run build` 통과

### 2026-04-12 (Windows 작업분)
- 홈 상단은 `DayMate` 대신 사용자 이름 중심 2줄 구조로 정리하고, 날짜/요일/시간은 이름 아래 줄로 이동
- 홈 중간의 `배너 준비 중입니다` placeholder 블록 제거
- 검증: `npm run build` 통과
- 다음 작업자 메모: 필요하면 추후 실제 배너 위치는 홈이 아닌 별도 영역에서 다시 설계

### 2026-04-12 (Windows 작업분)
- 홈 위계 조정: `오늘 할일` 카드를 더 강하게 올리고, `오늘의 명언`은 가벼운 분위기 카드로 정리
- 홈 구성 모달에 `기본 추천 복원` 버튼을 추가해 기본 섹션 표시값/순서로 즉시 복원 가능
- 검증: `npm run build` 통과
- 다음 작업자 메모: 실제 홈 첫 화면 체감에서 `오늘 할일` 강조 강도가 적절한지 확인

### 2026-04-12 (Windows 작업분)
- 홈 구성 모달이 하단 네비게이션에 가리던 문제를 줄이기 위해 모달 전체와 하단 힌트를 약 100px 위로 올림
- 홈 구성 마지막 섹션 접근성은 스크롤 힌트 + 추가 하단 여백 기준으로 조정
- 검증: `npm run build` 통과
- 다음 작업자 메모: 실제 모바일에서 오프셋이 과하면 `HOME_MODAL_NAV_CLEARANCE`만 조정하면 됨

### 2026-04-12 (Windows 작업분)
- 홈 구성 모달 하단에 `아래로 더 내려서 보기` 힌트를 추가해 마지막 섹션이 숨은 것처럼 보이던 문제를 완화
- 신규 사용자 기본 홈에서는 `목표 바로가기`, `자산 브리핑`을 기본 off로 조정
- 기존 사용자 `dm_home_prefs` 저장값은 그대로 유지
- 검증: `npm run build` 통과

### 2026-04-12 (Windows 작업분)
- **투자 기록 구조 확장**: BUY/SELL에는 수량과 체결 단가를 저장하고 총액을 자동 계산하도록 변경
- **투자 허브 개선**: 브리핑 종목에서 바로 기록 탭으로 넘기고, 기록 상세에서 원본 기록 수정 지원 추가
- 투자 기록은 기록 당시 시세/보유현황 스냅샷을 함께 저장하도록 확장
- 검증: `npm run build` 통과
