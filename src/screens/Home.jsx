...
export default function Home({ user, goals, setGoals = () => {}, lifeGoals = [], setLifeGoals = () => {}, isMyTab = false, todayData, plans, onToggleTask, onSetTodayTasks, habits, setHabits, onToggleHabit, onOpenDate, onOpenDateMemo, installPrompt, handleInstall, showInstallBanner, dismissInstallBanner, isIOS, isKakao, isStandalone, scores, event, inviteBonus, onOpenChat, isDark, setIsDark, getValidGcalToken, myRank, onOpenStats, recurringTasks, setRecurringTasks, someday, setSomeday, onLuckyXp, onOpenGoalsHub, onOpenSettings, invitePromptCode, recentInviteReward, onOpenInviteFlow, onDismissInvitePrompt, onDismissInviteReward, levelUpInfo, onDismissLevelUp, communityEventsToday = [], communityEventChecks = {}, onToggleCommunityEvent, myChallenges = [], onOpenChallengeHub, onOpenChallengeItem, telegramCfg, onOpenPortfolio, onAddMemo, onUpdateMemo, onDeleteMemo, onToggleMode, businessCard, setBusinessCard, authUser }) {
  ...
  const [newLifeInput, setNewLifeInput] = useState('');
  const [editingYearGoalsState, setEditingYearGoalsState] = useState(false);
  const [yearDraft, setYearDraft] = useState([]);
  const [newYearInput, setNewYearInput] = useState('');
  const [editingMonthGoalsState, setEditingMonthGoalsState] = useState(false);
  ...
  const downloadBcBlob = (blob) => {
    ...
    a.href = url; a.download = `${(user?.name || '내') + '_명함'}.jpg`;
  ...
  const saveMonthGoalsFn = () => {
    const final = [...monthDraft, ...(newMonthInput.trim() ? [newMonthInput.trim()] : [])].filter(g => g.trim()).slice(0, 5);
    const next = setMonthGoalsUtil(goals, getCurrentGoalMonthKey(), final);
    setGoals(next);
    store.set('dm_goals', next);
    setEditingMonthGoalsState(false);
    setNewMonthInput('');
  };
    ...
    <div style={S.content}>
                ...
                <div style={{ borderRadius: 14, border: '1px solid var(--dm-border)', background: 'var(--dm-card)', padding: '12px 14px' }}>
                  ...
                  {businessCard?.photoUrl ? (
                    ...
                    <button onClick={bcPickPhoto} disabled={bcUploading || !authUser}
                      style={{ width: '100%', aspectRatio: '1.6 / 1', borderRadius: 12, border: '1.5px dashed var(--dm-border)', background: 'var(--dm-input)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: authUser ? 'pointer' : 'default', color: 'var(--dm-muted)', opacity: authUser ? 1 : 0.5 }}>
                      <span style={{ fontSize: 26 }}>{bcUploading ? '⏳' : '📇'}</span>
                      <span style={{ fontSize: 12, fontWeight: 700 }}>{bcUploading ? '업로드 중...' : authUser ? '명함 사진 추가' : '로그인 후 사용 가능해요'}</span>
                    </button>
                  )}
      ...
      {false && quickMemoOpen && (
        ...
        >
          <div
            onClick={e => e.stopPropagation()}
            style={{ width: '100%', background: 'var(--dm-card)', borderRadius: '20px 20px 0 0', padding: '18px 16px 12px', border: '1px solid var(--dm-border2)', maxHeight: '75vh', display: 'flex', flexDirection: 'column', gap: 12 }}
          >
            ...
            <div style={{ overflowY: 'auto', flex: 1, minHeight: 0 }}>
              <MemoTimeline
                memos={todayData?.memos || []}
                onAdd={onAddMemo}
                onUpdate={onUpdateMemo}
                onDelete={onDeleteMemo}
                placeholder="메모 입력 후 Enter 또는 + 버튼"
              />
            </div>
            <button
              onClick={() => setQuickMemoOpen(false)}
              style={{
                background: 'rgba(75,111,255,0.15)', border: '1.5px solid rgba(75,111,255,0.4)',
                color: '#6C8EFF', borderRadius: 12, padding: '12px 0', fontSize: 14,
                fontWeight: 900, cursor: 'pointer', width: '100%', fontFamily: 'inherit',
                marginBottom: 12,
              }}
            >✓ 저장 완료</button>
          </div>
