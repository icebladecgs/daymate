    ...
    <div style={S.content}>
      ...
      <div onClick={() => {
        ...
        <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={S.sectionEmoji}>🔮</span>오늘의 운세
          <span style={{ fontSize: 11, color: "var(--dm-muted)", fontWeight: 700 }}>{fortuneOpen ? "▾" : "▸"}</span>
          {fortuneData && !fortuneOpen && (() => {
            const total = Math.round(((fortuneData.overall||3)+(fortuneData.money||3)+(fortuneData.health||3)+(fortuneData.relation||3))/4*20);
            return <span style={{ fontSize: 11, color: "#FCD34D", fontWeight: 900 }}>{total}점</span>;
          })()}
        </span>
      </div>
